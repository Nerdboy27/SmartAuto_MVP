...
let db, autosCollection;

async function connectDB() {
    try { await client.connect(); db = client.db('SmartAutoDB'); autosCollection = db.collection('autos'); } catch (err) {}
}
connectDB();
...
io.on('connection', (socket) => {
    socket.on('register_device', async (autoId) => {
        ...
        if (autosCollection) {
            ...
            if (!autoData) { await autosCollection.insertOne({ autoId: autoId, totalEarnings: 0 }); socket.emit('update_total', { total: 0 }); }
            else { socket.emit('update_total', { total: autoData.totalEarnings }); }
        }
    });
});
...
app.post('/webhook/payment', async (req, res) => {
    ...
    if (req.headers['x-razorpay-signature'] !== digest && WEBHOOK_SECRET !== 'DEBUG') { return res.status(403).send('Invalid signature'); }
    ...
    try {
        ...
        let newTotal = 0;
        if (autosCollection) {
            const updateResult = await autosCollection.findOneAndUpdate({ autoId: autoId }, { $inc: { totalEarnings: amountInRupees } }, { returnDocument: 'after', upsert: true });
